...
describe('observability', () => {
    ...
    describe('Health check endpoints', () => {
        test('GET /health returns OK', async () => {
            ...
            const app = createApp();
            const res = await (0, supertest_1.default)(app).get('/health').expect(200);
        ...
        test('GET /healthz includes database status', async () => {
            const createApp = require('../../src/app').default;
            const app = createApp();
            const res = await (0, supertest_1.default)(app).get('/healthz').expect(200);
            expect(res.body).toHaveProperty('ok', true);
        ...
        test('GET /ready returns readiness status', async () => {
            const createApp = require('../../src/app').default;
            const app = createApp();
            const res = await (0, supertest_1.default)(app).get('/ready').expect(200);
            expect(res.body).toHaveProperty('ready', true);
        });
        test('GET /live returns liveness status', async () => {
            const createApp = require('../../src/app').default;
            const app = createApp();
            const res = await (0, supertest_1.default)(app).get('/live').expect(200);
    ...
    describe('Request correlation IDs', () => {
        ...
        test('Each request gets unique correlation ID', async () => {
            const createApp = require('../../src/app').default;
            const app = createApp();
            const res1 = await (0, supertest_1.default)(app).get('/health');
            const res2 = await (0, supertest_1.default)(app).get('/health');
    ...
    describe('Health checks across endpoints', () => {
        test('All health endpoints are accessible without authentication', async () => {
            ...
            for (const endpoint of endpoints) {
                const res = await (0, supertest_1.default)(app).get(endpoint);
