...
import { decodeCursor } from '../../src/lib/pagination';
...
describe('pagination integration', () => {
  ...
  describe('GET /api/venues pagination', () => {
    ...
    test('GET /api/venues?limit=1 returns 1 venue with next_cursor', async () => {
      ...
      const createApp = require('../../src/app').default as () => any;
      const app = createApp();
      const res = await request(app).get('/api/venues?limit=1').expect(200);
    ...
    test('GET /api/venues?limit=1&cursor=X returns next page', async () => {
      ...
      // Get first page
      const page1 = await request(app).get('/api/venues?limit=1').expect(200);
    ...
    test('pagination cursor chain returns correct ordering', async () => {
      ...
      const allRes = await request(app).get('/api/venues?limit=100').expect(200);
      const allVenues = allRes.body.data.map((v: any) => v.id);
      ...
      for (let i = 0; i < allVenues.length; i++) {
        const pageRes = cursor
          ? await request(app).get(`/api/venues?limit=1&cursor=${cursor}`).expect(200)
          : await request(app).get('/api/venues?limit=1').expect(200);
  ...
  describe('GET /api/pitches pagination', () => {
    ...
    test('GET /api/pitches?limit=2 returns 2 pitches with pagination info', async () => {
      ...
      const createApp = require('../../src/app').default as () => any;
      const app = createApp();
      const res = await request(app).get('/api/pitches?limit=2').expect(200);
  ...
  describe('GET /api/sessions pagination', () => {
    ...
    test('GET /api/sessions?limit=1 returns 1 session with next_cursor if more exist', async () => {
      ...
      const allRes = await request(app).get('/api/sessions?limit=100').expect(200);
      const totalSessions = allRes.body.data.length;
      
      // Get first page
      const res = await request(app).get('/api/sessions?limit=1').expect(200);
      expect(res.body.data.length).toBe(1);
