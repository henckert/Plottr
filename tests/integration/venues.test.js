...
describe('venues integration', () => {
    ...
    test('GET /api/venues/:id returns single venue', async () => {
        ...
        const listRes = await (0, supertest_1.default)(app).get('/api/venues').expect(200);
        expect(listRes.body.data.length).toBeGreaterThan(0);
        const venueId = listRes.body.data[0].id;
        // fetch single venue
        const res = await (0, supertest_1.default)(app).get(`/api/venues/${venueId}`).expect(200);
        expect(res.body).toHaveProperty('data');
    ...
    test('POST /api/venues creates venue with 201', async () => {
        ...
        const clubRes = await (0, supertest_1.default)(app).get('/api/venues').expect(200);
