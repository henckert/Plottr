...
describe('sessions integration', () => {
    ...
    test('GET /api/sessions/:id returns single session', async () => {
        ...
        const listRes = await (0, supertest_1.default)(app).get('/api/sessions').expect(200);
        expect(listRes.body.data.length).toBeGreaterThan(0);
        const sessionId = listRes.body.data[0].id;
        // fetch single session
        const res = await (0, supertest_1.default)(app).get(`/api/sessions/${sessionId}`).expect(200);
        expect(res.body).toHaveProperty('data');
    ...
    test('GET /api/sessions/:id returns 404 for non-existent session', async () => {
        ...
        const res = await (0, supertest_1.default)(app).get('/api/sessions/99999').expect(404);
    ...
    test('GET /api/sessions filters by pitch_id', async () => {
        ...
        const filteredRes = await (0, supertest_1.default)(app)
            .get('/api/sessions')
    ...
    test('POST /api/sessions creates session with 201', async () => {
        ...
        const pitchRes = await (0, supertest_1.default)(app).get('/api/pitches').expect(200);
